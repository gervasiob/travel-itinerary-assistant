...
abstract class AuthManager {
  ...
  Future sendEmailVerification({required fa.User user}) async =>
      user.sendEmailVerification();
  Future refreshUser({required fa.User user}) async => user.reload();
}

// Email/password authentication mixin
mixin EmailSignInManager on AuthManager {
  Future<fa.User?> signInWithEmail(
    BuildContext context,
    String email,
    String password,
  );

  Future<fa.User?> createAccountWithEmail(
    BuildContext context,
    String email,
    String password,
  );
...
mixin FacebookSignInManager on AuthManager {
  Future<fa.User?> signInWithFacebook(BuildContext context);
}

// Microsoft Sign-In authentication (Azure AD)
mixin MicrosoftSignInManager on AuthManager {
  Future<fa.User?> signInWithMicrosoft(
    BuildContext context,
    List<String> scopes,
    String tenantId,
  );
...
mixin GithubSignInManager on AuthManager {
  Future<fa.User?> signInWithGithub(BuildContext context);
}
